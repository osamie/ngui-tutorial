...
#if !UNITY_EDITOR && (UNITY_IOS || UNITY_ANDROID || UNITY_WP8 || UNITY_WP_8_1 || UNITY_METRO || UNITY_BLACKBERRY)
#define MOBILE
#endif
...
using System.Text;
/*** ZYNGA ADDED MODIFIED ***/
#if UNITY_METRO && !UNITY_EDITOR
using UnifiedIO;
using Thread = Zynga.Casino.Utility.WPCompat.Thread;
#endif
/*** END ZYNGA ADDED MODIFIED ***/
...
public class UIInput : MonoBehaviour
{
    ...
    /* ZYNGA ADDED MODIFIED */
    public enum InputLineType { None, SingleLine, MultiLine }
    public InputLineType forceInputFieldType{get; set;}

    /**
     * Determines the type of native input field to create
     * Returns the type of the input line expected for TouchKeyboard.open parameter
     *  true  - Singleline
     *  false - Multiline
     **/
    private bool inputFieldType()
    {
        if (this.forceInputFieldType != InputLineType.None)
        {
            return (this.forceInputFieldType == InputLineType.MultiLine);
        }
...
/**** ZYNGA MODIFIED ****
 * BlackBerryPlayer deprecated with Unity 5.4.3
 * 
...
**** END ZYNGA MODIFIED ****/
    ...
    /* ZYNGA ADDED MODIFIED */
    // FIX FOR VIRTUAL KEYBOARD ISSUE WINDOWS
    // JIRA: POK-4026
    // Shifts view to ensure the UIInput field is visible when
    // the virtual keyboard is shown.
#if UNITY_WINRT && !UNITY_EDITOR
	public bool isVirtualKeyboardOffsetEnabled
	{
		get { return mVirtualKeyboardOffsetEnabled; }
		set { mVirtualKeyboardOffsetEnabled = value; }
	}
	private bool mVirtualKeyboardOffsetEnabled = true;

	// Any y values below this threshold require offsetting for
	// windows on screen keyboard.
	private const float CameraOffsettingYThreshold = 0.15f;

	private bool isCameraPositionOffset;
	private Vector3 preOffsetPosition;
#endif
	...
	protected virtual void OnSelect (bool isSelected)
	{
		...
		// the virtual keyboard is shown.
#if UNITY_WINRT && !UNITY_EDITOR
		if (mVirtualKeyboardOffsetEnabled)
		{
			var camera = UICamera.currentCamera;
			bool underTheCamera = false;

			var transform = this.transform;
			while (transform.parent != null && transform.parent != camera.transform)
			{
			...
			if (isSelected)
			{
				Bounds inputBounds = NGUIMath.CalculateAbsoluteWidgetBounds(this.transform);
				float minGlobalPosY = inputBounds.center.y - inputBounds.extents.y;

				// If the input is near the bottom of the screen, shift upwards
				// while selected, so the on screen keyboard doesn't cover the input.
				// Check using global y pos, where y=0.0f is the center of the screen
				if (minGlobalPosY <= UIInput.CameraOffsettingYThreshold)
				{
					preOffsetPosition = targetTransform.position;
					var targetPos = preOffsetPosition;
					float offsetY = minGlobalPosY - UIInput.CameraOffsettingYThreshold;

					if (underTheCamera)
					{
						// Move the hierarchy upward
						EnableAnchors(targetTransform, false);
						targetPos.y -= offsetY;
					}
					else
					{
						// Move the camera hierarchy down, effectively displaying popups higher
						targetPos.y += offsetY;
					}

					targetTransform.position = targetPos;
					isCameraPositionOffset = true;
				}
			}
			else if(isCameraPositionOffset)
			{
				targetTransform.position = preOffsetPosition;
				if (underTheCamera)
				{
					EnableAnchors(targetTransform, true);
				}

				isCameraPositionOffset = false;
			}
		}
#endif
        // FIX FOR VIRTUAL KEYBOARD ISSUE WINDOWS
    ...
    /// <summary>
    /// Helper function to enable/disable anchors.
    /// </summary>

    private void EnableAnchors(Transform targetTransform, bool enable)
    {
        var anchor = targetTransform.gameObject.GetComponentsInChildren<UIAnchor>();
        if (anchor != null)
        {
...
/**** ZYNGA MODIFIED ****
 * WP8 Player deprecated with unity 5.4.3
 ...
 **** END ZYNGA MODIFIED ****/               
				...
				)
				{
					...
					mKeyboard = (inputType == InputType.Password) ?
						...
						TouchScreenKeyboard.Open(val, kt, !inputShouldBeHidden && inputType == InputType.AutoCorrect,
							this.inputFieldType() && !hideInput, false, false, defaultText);
					/* END ZYNGA ADDED MODIFIED */
 #if UNITY_METRO
			...
			if (mKeyboard != null)
			{
				...
				{
					//Can't assume that text will be 1 char long.
					//If the first char is a backspace, DoBackspace will clear selected text (if any)
 ...
 /*
 * ZYNGA ADDED MODIFIED
 * POK-4400: iOS keyboard should not submit when the onscreenkeyboard's hide button is pressed
 */
				if (mKeyboard.done || !mKeyboard.active)
				{
#if UNITY_IOS
					if (!mKeyboard.wasCanceled && (mKeyboard.done || mKeyboard.active)) Submit();
#else
...
/**** ZYNGA MODIFIED ****
 * Web player deprecated with Unity 5.4.3
 *
 ...
 **** END ZYNGA MODIFIED ****/);
	...
	protected virtual void Insert (string text)
	{
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
			...
			{
				/* ZYNGA ADDED MODIFIED */
				// DoBackspace doesn't update the local string builder
				// So in the end it does nothing...
				#if UNITY_METRO
					if (sb.Length > 0)
					{
			...
			/* ZYNGA ADDED MODIFIED */
			// Fix for POK-4190: Return key is appending extra chars to fields (bad for passwords)
			#if UNITY_METRO
				if (c == '\r' || c == '\n')
				{
					continue;
	...
	protected virtual void OnPress (bool isPressed)
	{
		...
		{
#if !UNITY_EDITOR && (UNITY_WP8 || UNITY_WP_8_1 || UNITY_METRO)
