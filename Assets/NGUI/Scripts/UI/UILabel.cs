...
public class UILabel : UIWidget
{
	...
	bool keepCrisp
	{
		get
		{
			if (trueTypeFont != null && keepCrispWhenShrunk != Crispness.Never)
			{
				...
				#if (UNITY_IOS || UNITY_ANDROID || UNITY_WINRT || UNITY_BLACKBERRY)
	...
	/******************* ZYNGA ADDED! *******************/
    /// <summary>
    /// This allows to change both the size and the font at the same time. It should be more efficient than 
    /// doing it one at a time and avoid implementing invalid sizes for a given font.
    /// </summary>
    /// <param name="trueTypeFont">True type font.</param>
    /// <param name="fontSize">Font size.</param>
    public void SetFontAndSize(Font trueTypeFont, int fontSize)
    {
       // If there is no font don't do any change
        ...
        int size = Mathf.Clamp(fontSize, 0, 256);
	...
	public void UpdateNGUIText ()
	{
		...
		NGUIText.fontScale = isDynamic ? mScale : ((float)mFontSize / mFont.defaultSize) * mScale;
		// ************** Zynga Added *************** //
		NGUIText.gameObject = this.gameObject;
		// *************** End Zynga Added ********** //
