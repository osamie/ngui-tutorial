...
public class UIAtlasMaker : EditorWindow
{
	...
	static int Compare (SpriteEntry a, SpriteEntry b)
	{
		...
		// LP-19377 - Atlas Maker fails packing atlases with really oblong objects
		// Original algorithm sorts by which sprite is bigger
		// Changed to sort by which one has the longest side
		int aPixels = Mathf.Max(a.width, a.height);
		int bPixels = Mathf.Max(b.width, b.height);
		/******************* END ZYNGA MODIFIED! **************/
...
#if (UNITY_ANDROID || UNITY_IOS || UNITY_WINRT) && !UNITY_EDITOR
...
/*** END ZYNGA ADDED MODIFIED ***/
	...
	static public void ExtractTextures (UIAtlas atlas, List<Texture> finalTextures, bool keepAtlasReadable = false)
	{
		ShowProgress(0f);
		...
		if (tex != null)
		{
			...
			foreach (UISpriteData es in sprites)
			{
				...
				foreach (Texture fs in finalTextures)
				{
					if (es.name == fs.name)
					{
						found = true;
				...
				if (!found)
				{
					System.Collections.Generic.IEnumerable<string> textureFiles = from file in Directory.GetFiles(Application.dataPath, es.name + ".*", SearchOption.AllDirectories) where !file.EndsWith(".meta") select file;
					if (textureFiles.Count() > 1)
					{
						Debug.LogWarning("More than one texture named '" + es.name + "' was found!");
					}
					else if (textureFiles.Count() == 0)
					{
						Debug.LogWarning("No textures named '" + es.name + "' were found!");
					}

					if (textureFiles.Count() >= 1)
					{
					string textureFile = textureFiles.ElementAt(0);
					string assetPath = "Assets" + textureFile.Replace(Application.dataPath, "").Replace('\\', '/');
					Texture texture = (Texture)AssetDatabase.LoadAssetAtPath(assetPath, typeof(Texture));
					if (texture != null) {
						finalTextures.Add(texture);
					}
					}
				}
			}
	...
	static public bool UpdateTexture (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		if (PackTextures(tex, sprites))
		{
			...
			UnityEngine.Windows.File.WriteAllBytes(newPath, bytes);
			#else
	...
	/******************* ZYNGA ADDED! *******************/
	/// <summary>
	/// Don't throw away the border and padding data when updating an atlas!
	/// We use it for Tiled sprites, and it won't change when we move the texture.
	/// </summary>
	protected void PreserveBordersAndPadding(List<UISpriteData> oldSprites, List<SpriteEntry> newSprites)
	{
		float count = newSprites.Count;
		int index = 0;
		...
		foreach (SpriteEntry newSprite in newSprites)
		{
			ShowProgress((index++) / count);
			...
			foreach (UISpriteData oldSprite in oldSprites)
			{
				...
				{
					newSprite.CopyBorderFrom(oldSprite);
					newSprite.SetPadding(oldSprite.paddingLeft, oldSprite.paddingBottom, oldSprite.paddingRight, oldSprite.paddingTop);
					break;
				}
			}
		}
	...
	void UpdateAtlas (List<Texture> textures, bool keepSprites)
	{
		...
		if (sprites.Count > 0)
		{
			...
			/******************* ZYNGA ADDED! *******************/
			PreserveBordersAndPadding(NGUISettings.atlas.spriteList, sprites);
			/******************* END ZYNGA ADDED *******************/
...
/*** END ZYNGA ADDED MODIFIED ***/
		...
		if (NGUISettings.atlas != null)
		{
			...
			else if (GUILayout.Button("View Sprites"))
			{
				...
				recreate = GUILayout.Button("Recreate Atlas (Slow)");
		...
		if (spriteList.Count > 0)
		{
			NGUIEditorTools.DrawHeader("Sprites", true);
			{
				...
				else if (recreate)
				{
					...
					ExtractTextures(NGUISettings.atlas, textures, true);
				...
				/******************* ZYNGA ADDED! *******************/
				else if (update || replace || recreate)
				/***************** END ZYNGA ADDED ******************/
				{
