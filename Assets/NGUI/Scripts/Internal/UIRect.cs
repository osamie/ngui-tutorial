...
public abstract class UIRect : MonoBehaviour
{
	...
	{
/********* ZYNGA MODIFIED **************/
        public Transform Target //we should use this everywhere
        {
            get
            {
                return target;
        ...
        [SerializeField] private Transform target; //serialized variable that is private (that has the same name so our data won't change)
        [System.NonSerialized] public Camera cachedCamera; //for now, we're not serializing the cached camera - we may want to revisit in the future for load times
/********* END ZYNGA MODIFIED **************/
...
/********* ZYNGA MODIFIED **************/
        ...
        {
            this.target = target;
            this.cachedCamera = target.GetComponent<Camera>();
        }

        public void UpdateCachedCamera()
        {
            if (target != null)
            {
                this.cachedCamera = target.GetComponent<Camera>();
...
/********* ZYNGA MODIFIED **************/
                ...
                {
                    return cachedCamera.GetSides(relativeTo);
                }
/********* END ZYNGA MODIFIED **************/
			}
			return null;
...
/********* ZYNGA MODIFIED **************/
    protected virtual void Awake() //updated cached camera component ( in case the target were somehow set in an unexpected way )
    {
        leftAnchor.UpdateCachedCamera();
        rightAnchor.UpdateCachedCamera();
        bottomAnchor.UpdateCachedCamera();
        topAnchor.UpdateCachedCamera();
    }
/********* END ZYNGA MODIFIED **************/
