...
static public class NGUIText
{
	...
	static public Dictionary<string, HashSet<int>> allowedFonts = null;
	// *************** end Zynga added ********** //
	...
	static public void Update (bool request)
	{
		...
		// ************** Zynga added *************** //
		#if VERIFY_FONTS && !UNITY_STANDALONE
		if (allowedFonts == null)
		{
			NGUIText.GenerateAllowedFonts();
		}

        if (dynamicFont != null && NGUIText.allowedFonts != null && NGUIText.gameObject != null)
		{
            HashSet<int> validFontSizes = null;
            if(!allowedFonts.TryGetValue(dynamicFont.name, out validFontSizes) ||
               validFontSizes == null)
            {
                UnityEngine.Debug.LogError("On NGUIText.gameObject='" + NGUIText.gameObject.name + "' font=" + dynamicFont.name + " is not allowed!", gameObject);
            }
            else if(!validFontSizes.Contains(finalSize))
            {
                StringBuilder stringBuilder = new StringBuilder("[");
                HashSet<int>.Enumerator enumerator = validFontSizes.GetEnumerator();
                ...
                UnityEngine.Debug.LogError("On gameObject='" + gameObject.name + "' font=" + dynamicFont.name + " of size=" + finalSize + " not allowed! validSizes=" + stringBuilder.ToString(), gameObject);
            }
		}
		#endif
		// *************** end Zynga added ********** //
	...
	// ************** Zynga added *************** //
	#if VERIFY_FONTS
	static private void GenerateAllowedFonts()
	{
		NGUIText.allowedFonts = new Dictionary<string, HashSet<int>>();
		foreach (Zynga.Casino.Font casinoFont in Zynga.Casino.Config.AppConfig.instance.allowedFonts)
		{
			string fontName = casinoFont.font.name;
			if (!NGUIText.allowedFonts.ContainsKey(fontName))
			{
				NGUIText.allowedFonts[fontName] = new System.Collections.Generic.HashSet<int>(casinoFont.sizes);
			}
			else
			{
				foreach (int fontSize in casinoFont.sizes)
				{
					if (!NGUIText.allowedFonts[fontName].Contains(fontSize))
					{
						NGUIText.allowedFonts[fontName].Add(fontSize);
					}
				}
			}
		}
