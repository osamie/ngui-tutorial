...
public class UIPopupList : UIWidgetContainer
{
		...
		if (mChild != null)
		{
			...
			if (isAnimated)
			{
				...
				for (int i = 0, imax = widgets.Length; i < imax; ++i)
				{
					...
					c.a = 0f;
					/******************* ZYNGA MODIFIED! *******************/
					TweenColor tc = TweenColor.Begin(w.gameObject, animSpeed, c);
					if(tc != null)
					{
						tc.method = UITweener.Method.EaseOut;
					}
					/******************* END ZYNGA MODIFIED ****************/
	...
	void AnimateColor (UIWidget widget)
	{
		...
		/******************* ZYNGA MODIFIED! *******************/
		TweenColor tc = TweenColor.Begin(widget.gameObject, animSpeed, c);
		if(tc != null)
		{
			tc.method = UITweener.Method.EaseOut;
		}
	...
	public void Show ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && atlas != null && isValid && items.Count > 0)
		{
			...
			/******************* ZYNGA ADDED! *******************/
			if(!foundSelectedItem && labels.Count > 0)
			{
				this.Highlight(labels[0], true);
			}
			/******************* END ZYNGA ADDED! ***************/
